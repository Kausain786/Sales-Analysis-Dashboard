<img width="1348" height="724" alt="Screenshot 2026-03-25 165052" src="https://github.com/user-attachments/assets/fc84a6f2-66ac-4b7b-b53a-fd2b77182293" />
<img width="1355" height="726" alt="Screenshot 2026-03-25 165126" src="https://github.com/user-attachments/assets/c96cd801-b439-470b-8dc4-5f7b5c391956" />
The dashboard helps in identifying high-performing products, regions, and sales channels, enabling better business decisions.

**Tools & Technologies Used**
Power BI (Data Visualization & Dashboarding)
Microsoft Excel (Data Source & Cleaning)

**Key Metrics (KPIs)**
Total Sales: 302.80M
Total Profit: 43.49M
Total Quantity Sold: 13,917
Total Orders: 2521
Profit Margin: 13.62%

**Dashboard Insights**
1. Regional Performance
Sales and profit are distributed across East, West, North, and South regions
Certain regions show higher profit margins indicating better efficiency
2. State-wise Profit Analysis
Top contributing states include:
Gujarat
Maharashtra
Uttar Pradesh
Profit contribution is visualized using a donut chart for easy comparison
3. Sales Trends (Time Analysis)
Monthly and quarterly analysis shows variation between 2023 and 2024
Seasonal trends can be identified for better planning
4. Product Performance
Products like Laptop and Tablet generate higher sales
Some products show declining trends indicating improvement opportunities
5. Salesperson Performance
Top performers:
Neha
Priya
Sneha
Helps in evaluating team performance
6. Discount vs Profit Analysis
Higher discounts lead to reduced profit margins
Useful for pricing strategy decisions
7. Sales by Channel
Channels analyzed:
Retail Store
Distributor
Online
Balanced contribution across all channels
8. Category-wise Sales
Categories:
Electronics
Accessories
Electronics contributes slightly higher revenue
9. City-wise Sales & Profit
Major cities:
Mumbai
Delhi
Bangalore
Helps identify top-performing markets
